...
function IFI() {
    const [ posts, setPosts, tokens ] = useState([])
    useEffect(() => {
        async function loadToken() {
            // let base64 = require('base-64');

            let url = 'http://127.0.0.1:8000/api/auth/token/obtain/';
            // let username = Buffer.from('umid').toString('base64');
            // let password = Buffer.from('joker82').toString('base64');
        ...
        async function loadData() {
            const posts = await fetch('http://127.0.0.1:8000/api/ifis.json', 
                method ='GET',
                mode = 'no-cors', 
                headers = {
                    // 'Authorization': `Token ${this.props.tokens}` ,
